...
function getProfileDedupKey(
  profile: Pick<
    ...
    'provider' | 'baseUrl' | 'apiKey' | 'model' | 'apiMode' | 'streamImages' | 'streamPartialImages'
  >,
) {
...
export function buildSettingsFromUrlParams(
  ...
  if (hasLegacyOpenAIParams) {
    ...
    if (streamPartialImagesParam !== null)
      profile.streamPartialImages = normalizeStreamPartialImages(streamPartialImagesParam)
