...
function createResponsesImageTool(
  ...
  const tool: Record<string, unknown> = {
    ...
    // 图生图（编辑）请求统一使用 size=auto：输出尺寸跟随参考图比例。
    // 实测部分兼容中转站对非 auto 尺寸的编辑请求会挂起/拒绝（1280x720、1024x1024 均不响应），
    // 而 auto 是官方文档化的合法值，对支持显式尺寸的服务商同样有效。
    size: isEdit ? 'auto' : params.size,
...
async function parseImagesApiStreamResponse(
  ...
  const actualParams = mergeActualParams(actualParamsList[0], images.length > 1 ? { n: images.length } : undefined)
  return {
    images,
    actualParams,
    actualParamsList,
...
function linkTaskSignal(controller: AbortController, signal?: AbortSignal) {
  if (!signal) return
  if (signal.aborted) {
...
async function callCustomHttpImageApi(
  ...
  try {
    ...
    if (submitMapping.taskIdPath && !taskId) {
      ...
      ;(err as Error & { rawResponsePayload?: string }).rawResponsePayload = JSON.stringify(submitPayload, null, 2)
      throw err
    }
