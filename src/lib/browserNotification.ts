...
export type BrowserNotificationPermissionResult =
  { ok: true } | { ok: false; reason: 'unsupported' | 'insecure' | 'denied' | 'default' | 'error'; error?: unknown }
export type BrowserNotificationReadiness =
  { ok: true } | { ok: false; reason: 'unsupported' | 'insecure' | 'denied' | 'default' }
...
export function showBrowserNotification(title: string, options?: NotificationOptions) {
  // Electron：只走主进程原生系统通知（点击聚焦窗口），避免与浏览器 Notification 双发
  if (isElectron() && window.electronAPI?.showNotification) {
    void window.electronAPI.showNotification(title, typeof options?.body === 'string' ? options.body : undefined)
    return true
  }

  const NotificationConstructor = getNotificationConstructor()
  if (!NotificationConstructor || !isSecureNotificationContext() || NotificationConstructor.permission !== 'granted')
    return false
