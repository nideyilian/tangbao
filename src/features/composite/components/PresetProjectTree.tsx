 ...
 * 输出目录、命名模板、渠道规格、启用范围全在后处理那套界面里 —— 那套界面才是这些参数的
 * 权威编辑面，树上一个都不放，避免同一个参数有两个入口而出现「在 A 改了、在 B 看不到」。
 *
 * ⚠️ 别把这句话读成「树是权威、后处理只能看」。恰恰相反：按中控台的定位
 * （2026-09-20 确认，见 `design-system/tangbao/pages/postprocess.md`），
 * **中控台是全部参数的统一编辑入口**，节点级覆盖也靠分区内的作用域选择器来改。
 * 树只在「水印归属」这一件事上承担节点级编辑，原因是它天然按产品 / 方向分组，
 * 看归属就得看树；换成下拉选节点反而更难用。其余参数的节点级编辑一律在中控台。
 *
