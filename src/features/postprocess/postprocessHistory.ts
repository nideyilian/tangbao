 ...
 * - **产出目标的配置语义不动**：这里是**记录**，不是配置。`savedTargetCollectionIds` 仍是全局一份 ——
 *   它的语义天生是「**这一批**要投到哪几个方向」（跨方向），逐方向各存一份会让「哪个值生效」
 *   需要递归推理（见 `lib/postprocessMedia.ts` 里该字段的注释）。杰哥 2026-09-23 选的就是这条。
