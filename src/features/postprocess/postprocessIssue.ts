...
export type PostprocessIssueCode =
   ...
   * 所属方向未在「启用范围」内。
   *
   * ⚠️ **2026-09-23 起不再产生**：产生它的那层白名单（项目树「后处理」列）已撤掉 ——
   * 方向参不参与自动后处理改由方向级开关负责（`PP-SCOPE-002`），自动/手动的分界由设置里的
   * 总开关承担。**码表条目必须留着**：落盘的方向历史里还有旧记录带着它，删掉会让那些记录
   * 渲染成「未知问题」。
   */
  | 'PP-SCOPE-001'
  /** 所属方向关闭了自动后处理 */
  ...
  /** 输出子目录创建失败 */
  | 'PP-DIR-004'
  /** 该渠道配的导出位置被开关**全部**停用，这一桶没写（不是故障，是用户自己关的） */
  | 'PP-DIR-005'
  /** 同名文件过多，分配不出文件名 */
  | 'PP-NAME-001'
  /** 图片写入失败 */
...
const ISSUE_TEMPLATES: Record<PostprocessIssueCode, IssueTemplate> = {
  ...
  'PP-SCOPE-001': {
    message: '所属方向未启用后处理',
    // 不再产生，所以说法改成陈述当时的规则；照做的动作只留仍然有效的那个（手动跑）
    hint: '这个方向当时不在「启用范围」里，所以没产出变体。这条规则已取消：现在所有方向都参与自动后处理，跑不跑由设置里的「自动后处理」开关决定；只想产这一次，就选中素材手动跑一次。',
    severity: 'skipped',
  },
  'PP-SCOPE-002': {
  ...
  'PP-DIR-005': {
    message: '导出位置已全部停用，这一批没有写出文件',
    hint: '这个渠道配的导出位置被「启用」开关关掉了（中控台「渠道与输出」→ 各渠道的开关）。要恢复就打开对应开关；如果本来就不想投这个渠道，取消勾选它的「参与产出」更省事 —— 那样连变体都不再生成。',
    // skipped 而不是 error：这是用户自己关的，不是故障。写成 error 会让「按批次跑完」被算成失败，
    // 也会让自动触发的播报把一次刻意停用报成异常。
    severity: 'skipped',
  },
  'PP-NAME-001': {
