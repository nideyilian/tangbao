...
| **自动后处理**：总开关默认**关**（`settings.autoPostprocess`）· 原「启用范围」白名单已撤（字段只剩无归属兜底）· 纯配置跳过不留档（R-90）                                                                          | `architecture-constraints.md` §4.3/§4.4.1/§4.4.2             |
| **方向级历史记录 / 取消导出**（两套记录分工 · 全是快照 · 取消不删文件 · ⚠️ R-95 白名单重启清空）                                                                                                                  | `architecture-constraints.md` §4.4.3                         |
| **分发 = 按排期日同级改名整装**（目录名 = 排期日 + 命名模板 · 恒定 move · 第 1 天原地不搬 · 复制开关已撤）                                                                                                        | `architecture-constraints.md` §4.4.4 / **ADR-0018 / TB-117** |
| **删除即永久删除**（回收站已撤除 · 四个删除入口统一走 `deleteAssets`：无冲突当场删、被别的任务/会话引用才弹确认 · 删任务卡 = 产出图随卡真删 · 存量 `trashed` 由 `CATALOG_MIGRATIONS` v2 恢复正常 · 误删不可恢复） | **ADR-0021 / R-105 / TB-127**                                |
...
- **⭐ 区分「复制件」与「重编码产出」只能靠字节**：两条路径的撞名后缀规则**完全一样**
  （都是 `-2`），看文件名分不出来；复制件与原型**字节完全相同**，重编码产出**字节必不同**。
  ⚠️ **2026-09-23 起分发不再产生复制件**（改成恒定 move：同盘 `renameSync`、跨盘 copy+unlink，
  见 ADR-0018）—— 看到「同目录同名不同后缀、字节完全相同」时，别再往分发上想，
  现在会这么干的只有导出链路（素材库导出 / 项目树导出）。
...
- **⭐ 给缓存/索引写「删除」时，键必须来自构造函数**（2026-09-23 实测，R-99）：`thumbnailCache` 的键是
  `` `${id}:${variant}` ``，而永久删除路径写的是 `thumbnailCache.delete(imageId)` ⇒ **一条都没删掉**，
  不报错、不退化；同一条路径连磁盘缩略图都没调过删除接口。**自查一句话**：删完立刻 `get` 回读一次
  （比读代码快）；**反向验证时「撤掉修复但守卫仍绿」就是这条**（R-85 / R-93 同族）。
  代价实例：用户看到「素材没了、缩略图还在」，已删的图还能被推回界面。
