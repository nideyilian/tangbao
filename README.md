...
- **集成与协作**：`tangbao://` 深链接可从任意软件打开素材/搜索/导入（OS 协议关联）；本地 HTTP API（token 鉴权）支持查询、推荐与写操作（创建项目、按路径导入）；MCP 服务把素材库检索与命令开放给 AI 工具；支持外部图片导入（工具栏/拖拽/深链接/API，内容哈希去重）；详情面板提供上下游衍生链；工具栏「查重」按感知哈希找出近似重复素材，保留一张其余一键回收站。
