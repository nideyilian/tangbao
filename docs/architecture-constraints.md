...
- 勾选**不是**产出目标，是**启用范围**（哪些方向参与*自动*后处理）：归属方向**自身或任一祖先**
  被勾才算启用；**没勾就不跑**（仅对自动触发成立，见 4.4.1）。
- **自动触发的产出目标 = 图片归属方向本身**（命名段取自归属方向，与勾选层级无关）；
  **手动触发的产出目标 = 「记住配置」那份清单**（`savedTargetCollectionIds`），
  可以跨出启用范围 —— 两者在 `taskPostprocess` 里按 `source` 分流，别合并。
- 无归属 → 退回全局勾选。
...
- **启用范围（`PP-SCOPE-001`）同样只拦自动**（2026-09-22）：它的定义也是「哪些方向参与自动后处理」，
  不该管用户手动选的那批目标 —— 手动要能跨产品产到一个还没参与自动产出的方向。
- **「记住的产出目标」只有手动读**（同上）：自动产出的去向仍是图片归属方向，
  不被那份清单悄悄改掉。杰哥原话：「我这个只针对于手动后处理，不需要改自动后处理的」。
- 三处判定的落点：`taskPostprocess.ts` 里 `savedTargets = source === 'manual' ? … : []`、
  `PP-SCOPE-001` 与 `PP-SCOPE-002` 两处 `input.source !== 'manual'`。**加第四处判定时照这个格局走。**
