...
### 4.3 产出目标：自动按归属、手动按「记住配置」（2026-09-23 重写）

- **自动触发的产出目标 = 图片归属方向本身**（命名段取自归属方向）；
  **手动触发的产出目标 = 「记住配置」那份清单**（`savedTargetCollectionIds`），
  可以跨产品 / 跨方向 —— 两者在 `taskPostprocess` 里按 `source` 分流，别合并。
- **无归属（手工拖入 / 旧数据）→ 退回 `selectedCollectionIds`**。
...
- 判定的落点：`taskPostprocess.ts` 里 `savedTargets = source === 'manual' ? … : []`、
  `PP-SCOPE-002` 那处 `input.source !== 'manual'`。**加新判定时照这个格局走。**
