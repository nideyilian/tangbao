...
### 改后处理的画面适配（裁剪 / 模糊 / 拉伸）—— 2026-09-21 起

**入口**：中控台 tab → 左树选作用域 → 「渠道与输出」分区 → 「画面适配」三选一。
**它是全局一套**：在哪个作用域点都一样，改一次所有渠道所有方向全都生效
（不像「导出位置」「参与产出」那样跟着左树作用域走）。这是刻意的 —— 适配方式是「这批素材长什么样」
的全局美学规格，与「画面方向」同层（都在那张渠道表上方）。

| 选项             | 画面 | 变形 | 代价                                                         |
| ---------------- | ---- | ---- | ------------------------------------------------------------ |
| 裁剪填满（默认） | 填满 | 不变 | 丢边缘内容（等比放大后从中心裁掉溢出）                       |
| 模糊填充         | 完整 | 不变 | 四周是原图模糊底，**对留白敏感的渠道可能不收**（先确认规则） |
| 拉伸铺满         | 完整 | 变   | 比例被改变（人像 / 产品会变形）                              |

- 落盘字段：`PostprocessMediaConfig.fitMode`（值域 `crop-fill` / `contain-blur` / `stretch`）。
  **没有方向级覆盖** —— `applyPostprocessOverride` 里是显式透传。⚠️ 那个函数返回的是**白名单对象**，
  以后加字段漏了它，下游拿到 `undefined`，渲染器抛「未知的背景适应模式」，**整批产出当场废掉**
  （`architecture-constraints.md` 4.2.1 第 ② 条那条链）。
- **默认值刻意保持 `crop-fill`**：它与这个开关存在之前的行为一致，升级不改任何人的产出观感。
  要换默认值等于改所有人的产出，得先问。
- Excel 等价路径：中控台导出 → 改 `naming` 表的 `fitMode` 行 → 导入。
  **英文枚举与中文标签都认**；填了不认识的值会报一条 reject 并回落默认值（不静默吞掉）。
- 查当前值（DevTools Console，渲染进程）：

  ```js
  const s = (await import('/src/storePostprocessMedia.ts')).usePostprocessMediaStore.getState()
  console.log(s.fitMode)
  ```

- 三种模式的**实际观感**只能跑一遍看：本机无法做像素级渲染验证（见长期记忆「本机无法做网页渲染验证」）。
...
5. **⚠️ `node .git/stage-my-hunks.mjs` 在本机跑不动**（2026-09-23 实测）：脚本里每一处
   `execFileSync('git', …)` 都报 `Error: spawnSync git EBUSY`（沙箱拦 spawn，与脚本/仓库无关）
   ⇒ 正解是**把 git 调用留在 Bash、脚本只做文本处理**（读 `git diff` 落盘的 patch → 筛 hunk →
   写新 patch → 再由 Bash `git apply --cached`）。
6. **⚠️ 我的行与对方的行「紧贴」时，按 hunk 筛一定会连带对方**：两张表相邻两行（本轮：
   BACKLOG 的 TB-119 贴 TB-118、RISK 的 R-99 贴 R-98）会被 git 并成**同一个 hunk**，
   这种「只插我那一行」必须改用**带上下文的插入补丁**，且上下文要取自**HEAD/索引那份内容**
   —— 工作区里中间已经夹着对方的行了。
   ⚠️ **补丁绝不能是零上下文**（`@@ -N,0 +N+1,1 @@`）：`git apply` 不靠行号定位，会把这一行插到
   **文件末尾**（本轮实测：BACKLOG 的表格行跑到第 5511 行、RISK 的 R-99 跑到第 193 行，
   而两个文件分别只有 5510 / 192 行）。自检一条命令：`git show :<file> | grep -n "<我的行前缀>"`。
7. **⭐ 更稳的做法：干脆不按 hunk 筛，在 HEAD 版上「重放」自己的改动**（2026-09-23 实测，TB-120）。
   只要你的改动**以新增整行 / 整段为主**，就可以完全绕开 hunk 边界问题 —— 把
   「HEAD 版 + 我的插入」拼成一份新内容、直接写进索引，**工作区一个字节都不用动**：

   ```bash
   F=docs/BACKLOG.md
   git show HEAD:$F > "$TMP/base.md"            # 落盘基线（重定向留在 Bash，别在脚本里 spawn git，见第 5 条）
   # 用 python 在 base.md 上做你那几处插入 → "$TMP/mine.md"（脚本只做文本处理）
   BLOB=$(git hash-object -w "$TMP/mine.md")
   git update-index --cacheinfo "100644,$BLOB,$F"
   git diff --cached --stat                     # 核对：行数应等于「我插了多少行」
   ```

   - 自检两条：`git diff --cached $F | grep -c "<对方条目标记>"` 必须是 **0**；
     提交后该文件在 `git status` 里**仍显示 `M`**（对方内容还在工作区）。
   - 适用条件：改动以**新增**为主。改了对方已有的行就不适用 —— 那种情况仍走第 6 条，
     或者干脆等对方提交（最省事）。
...
## 十四·五、并行写线在场时，如何自证「测试失败不是我的」（2026-09-21 实测定稿）

**症状**：你只动了几个文件，跑测试却出现与它们无关的失败；`git status` 里还冒出你从没碰过的 `M` 文件。

**根因**：本仓「同一时间只有一条写线」是约定（R-01），但**约定不等于现实** —— 另一条会话
或另一个编辑器窗口随时可能在同一个工作区里写文件。表现就是「你刚提交完，工作区又长出几个未提交改动」。

**判定步骤（三步，别跳）**

1. **看清是谁**：`git status --porcelain` + `git diff --stat`，逐个确认失败用例关联的文件
   是否在你的 add 列表里。
2. **用 HEAD 版对照取证，不要换文件**（换入换出会撞上对方正在写的那一刻）：

   ```bash
   git show HEAD:<失败的源文件> | grep -n "失败的钩子"     # HEAD 版有没有
   grep -n "失败的钩子" <失败的源文件>                     # 工作区版还有没有
   git diff <失败的源文件> | grep -E "^-.*失败的钩子"       # 是不是「对方刚删掉的」
   ```

   实例（2026-09-21）：`ConsolePostprocessSections.test.tsx` 两个用例找不到
   `[data-testid="channel-output-dir-baidu-0"]` → HEAD 版有 3 处 `data-testid`、
   工作区版 0 处、`git diff` 显示正是那批未提交改动删掉的 ⇒ **归因对方，与我无关**。

3. **报告而不是动手**：不要为了让测试变绿去改对方的文件 —— 那会把两条写线缠在一起。
   把「哪个文件、哪个钩子、是谁的改动」写进交付说明，由人裁决先后。

**顺带一条纪律**：提交前一律跑一次完整 `npm run verify`。工作区里可能躺着别人的改动，
不跑就等于把「未验证的状态」当成自己的基线。
...
> ⚠️ **只喂 `format:check` 的实际范围**：`src/**/*.{ts,tsx,css}` + `electron/**/*.ts` +
> 根目录 `*.{js,json,md}`。**别把 `docs/**` 丢进去** —— 它在门禁之外，而 prettier 会按自己的
> 规则**重排整篇的表格对齐**：2026-09-22 实测，只往 `BACKLOG.md` 追加一段 TB-104，跑完
> `--write` 得到 **215 增 / 160 删**，全是与本轮无关的排版 diff。真要对 docs 排版，单独开一次
> 提交，别混进功能改动。（同理：临时脚本一律落 `%TEMP%`，别留项目根。）
...
| 守什么                       | 用例                                                                 |
| ---------------------------- | -------------------------------------------------------------------- |
| 手动侧拿到了新能力           | 「手动跑：记住的目标跨出启用范围也照产」                              |
| 自动侧没被顺手改掉（防回退） | 「自动跑不读记住的目标」，断言解析的仍是归属方向                     |
| 自动侧原行为仍在（防回退）   | 「自动跑：方向级开关关着 → 记 `PP-SCOPE-002` 且不留记录」             |

⚠️ **2026-09-23 起「启用范围」那层门已撤**（原表里守 `PP-SCOPE-001` 的两条随之作废）：
自动侧的判定只剩「总开关（`settings.autoPostprocess`，默认关）+ 方向级开关（默认开）」，
手动侧一道都不受。**配方不变** —— 仍然是「把该字段的全部读点列出来、逐个标它属于哪条路」，
只是这次要判的字段从 `selectedCollectionIds` 换成了 `settings.autoPostprocess`。
