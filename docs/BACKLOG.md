...
- **未做 / 说明**
  ...
  5. **产出目标的配置语义仍然不动**（杰哥选 A）：历史里的是**记录快照**，`savedTargetCollectionIds`
     ...
     → **2026-09-24 已回答并落地：见 TB-126**。答案是「按**图的归属方向**取那一份，不是按目标方向」——
     「A 方向设了产出到 X、Y」只对**属于 A 的图**生效；X 那份只管属于 X 的图，两份不会互相打架。

---

## TB-126 产出目标按方向各存一份：设置时跨产品、配置归「当时所在的文件夹」（2026-09-24 阿伟）

**报障**（杰哥原话）：「为什么产出目标这个功能还是按照全局的，我设置了一个方向的，结果全局都是这个设置，
我要的是设置的时候可以跨产品、跨方向，但配置是归于当时设置时所选的方向的」。

**根因**：那份清单只存了一份（`storePostprocessMedia.ts` 顶层的 `savedTargetCollectionIds`），全库共用；
判定里它的语义是「非空就对**所有**图生效」（原 `directionTargets.ts:38`、原 `taskPostprocess.ts:361`）。
不是漏写 —— 2026-09-23 刻意做成全局一份（理由还留在 `lib/postprocessMedia.ts` 与 `postprocessHistory.ts`
的注释里），本轮按杰哥的新要求改成「按文件夹各一份」。

**归属键（他拍板的）**：**素材库当前所在的文件夹**（`useAssetLibraryStore.scope` 指向的那条，
产品线 / 产品 / 方向任意一层）。那个按钮是常驻的、不跟选中素材走，所以 scope 是唯一可行口径。
另两个候选没被采纳：选中素材的归属方向（要求先选素材）、挪到项目树上（入口换地方，改动最大）。

**做法**（口径细节见 `docs/architecture-constraints.md` §4.3）

| 落点                                        | 改动                                                                                                                                                                                                                                                                |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/postprocessMedia.ts`                    | 新增 `savedTargetsByFolder: Record<string, string[]>`（键 = 节点 id）；`savedTargetCollectionIds` 降级为「兜底那一份」；`POSTPROCESS_FIELD_GROUP` 两份都归 `tree`；`applyPostprocessOverride` 显式透传两份                                                             |
| `storePostprocessMedia.ts`                   | 默认值 / 归一化（丢空列表的键、去重去空）/ `partialize` 白名单 / 快照；action 换成 `setSavedTargetsForFolder(scopeId \| null, ids)` 与 `clearSavedTargetsForFolder`（`null` = 写兜底那份；传 `[]` = 删键）                                                             |
| `features/postprocess/directionTargets.ts`   | **唯一实现**：新增 `resolveEffectiveSavedTargets`（沿「自己 → 祖先」找最近的命中）；`resolveImageTargetDirectionIds` 按它取用；输入加 `collections`                                                                                                                   |
| `features/postprocess/taskPostprocess.ts`    | **删掉自己那份手写口径**，改调 `resolveImageTargetDirectionIds`（两边口径从此不可能分叉）；循环外建一次 `collectionsById`（逐图建 Map 会把同一棵树解析几百遍）                                                                                                       |
| `PostprocessTargetsDialog.tsx`               | 顶部写明「这份配置归到：X / Y / Z」（继承来的另写「继承自某层，保存后写在当前文件夹上」）；草稿初值取**生效值**（本层没设、上层设过时显示继承来的那份）；「恢复按归属」只清本层                                                                                       |
| `AssetLibraryToolbar.tsx`                    | 按钮上的数字改成「当前范围生效的那份有几个方向」                                                                                                                                                                                                                    |
| `assetLibrary/scopeLabel.ts`（新）           | 「当前范围叫什么」从 `AssetLibraryWorkspace.tsx` 的私有函数提出来共用（弹窗要说清「你现在不在具体文件夹里」）                                                                                                                                                        |

**语义三条（别混）**

1. **取用规则只有一条**：沿「归属方向 → 它的祖先」向上找第一个设过的文件夹，最近的一环说了算
   （在「产品」层设的一份被其下方向继承，越具体越优先）；链上一份都没设 → 按归属方向产出（老行为）。
2. **兜底那一份只对「没有归属」的图生效**：素材库停在「全部素材 / 收藏 / 未整理 / 标签」时设置写它。
   让它对有归属的图生效 = 回到本次报障的 bug（有两条带 ⭐ 的回归用例守着）。
3. **自动后处理两份都不读**（口径没变）：去向仍是图片归属方向。

**迁移**：上一版那份全局配置**不折算**到任何文件夹（当初就是「全库一套」的意思，折算到哪条都是猜），
它留在兜底位上；用户在需要的文件夹下各设一次。**不 bump `persist.version`**（纯新增 + 缺字段回落
= 旧行为，没有需要折算的旧语义）。

**验收标准（可测，已在用例里钉住）**

1. 在 A 方向设的清单**不影响** B 方向手动跑（各按各的）；
2. 在「产品」层设的一份被其下方向继承；自己设过则自己那份优先；
3. 链上一份都没设 → 按图片归属产出（与改动前一致）；
4. 兜底那一份只对无归属的图生效；
5. 素材库不在具体文件夹里时写兜底那份，不碰任何文件夹的键；
6. 弹窗带入的是**生效值**（含继承来的），保存写在本层；本层从没写过时不显示「恢复按归属」；
7. 自动触发两份都不读（防回退）；
8. `npm run verify` 全绿。

**验收证据（2026-09-24）**

- `npm run verify` **全绿**：tsc 双端零错误 / eslint 零告警 / prettier 全 unchanged /
  **270 文件 3298 用例通过**。
- 用例增量：`directionTargets.test.ts` 8 → **17**（按文件夹取用 / 祖先继承 / 越具体越优先 /
  兜底不越界 / 不传树不继承 / 生效值含落点 / 自动不读两份…）；`PostprocessTargetsDialog.test.tsx`
  8 → **12**（界面写明归属 / 换方向是另一份 / 不在文件夹里写兜底 / 继承来的带入且保存写本层 /
  恢复按钮的显示条件）；`storePostprocessMedia.test.ts` +3（两份互不覆盖 / 删键 / 归一化删空键）；
  `taskPostprocess.test.ts` +1（「别的方向设的目标不影响这一条」）；`AssetLibraryToolbar.test.tsx` +1
  （按钮数字跟当前范围走）；`postprocessMedia.test.ts` 与 `treeConfigBundle.test.ts` 各改 1 条覆盖新字段。
- **反向验证两轮，全部精确命中**（每轮改完已回读校验复原，复原后 111 用例全绿）：
  1. 把兜底那份改回「对所有图生效」（= 本次要修的那个 bug 行为）→ **1 failed**，恰好是
     「⭐ 兜底那份不影响有归属的图」：`expected [ 'dir-b1' ] to deeply equal [ 'dir-a1' ]`，其余 48 条照过；
  2. 取用时**不传 `collections`**（模拟漏传那个可选参数，即 R-104）→ **2 failed**，恰好是两条继承用例
     「⭐ 在「产品」层设的一份，被它下面所有方向继承」与「越具体越优先」：
     `expected [ 'dir-a1' ] to deeply equal [ 'dir-b1' ]` / `expected [ 'dir-a2' ] to deeply equal [ 'dir-b1' ]`，其余 34 条照过。
- 配置包同步：`TreeConfigDefaults` / `docs/examples/*.schema.json` / `*.example.json` / `docs/config-spec.md`
  四处已跟上（纯新增字段，**不 bump 结构版本**，见规范 §七）。
- 配方与新增风险：runbook §20 补了「第二例：map 型字段的三处特有坑」；RISK 新增 **R-104**
  （取用漏传 `collections` = 静默降级成「不继承」），并更新 **R-90** 的落点（那段代码已搬进 `directionTargets.ts`）。

**未做 / 说明**

1. **本机无法做渲染验证**（`.env` 已知限制）⇒ 界面改动只做了组件级用例 + 规则推导，
   **未经真机观感确认**。请在运行中的应用里过一眼两处：素材库工具栏按钮上的数字是否随当前方向变、
   弹窗顶部那行「这份配置归到：…」是否读得通。
2. **上一版已设的那一份不会自动分配**到任何方向：它留在兜底位（只对无归属的图生效）。
   要按方向用，就在每个需要的方向下各设一次。
3. 本次只动「手动跑」的取用路径，**自动后处理一个字没改**。

---

## TB-127 撤除素材回收站：删除即永久删除（2026-09-24 阿伟）

**背景**：杰哥 2026-09-24 反馈「这些项都帮我去掉，特别是回收站，导致我当前删除时出现了非常多问题，
去掉后，删除就是真正的删除就行」。**当场确认范围 = 只撤回收站**，保留
「全部素材 / 最近生成 / 收藏 / 未整理」四项原样。决策见 **ADR-0021**（同时**作废 ADR-0019**）。

**为什么「非常多问题」**：同一个「删除」，后果取决于入口 —— 卡片右键是软删（图还在，只把状态标成
trashed），得再进回收站点一次「永久删除」才是真删。用户看到的是「删了没删掉」（封面照旧、角标照旧）；
而**唯一通往真删的路藏在回收站里**。软删还额外带出退槽、孤儿组放行、六处文案改口径、磁盘不释放等一串复杂度。

**做了什么**

1. 侧边栏撤「回收站」项（`SYSTEM_SCOPES`）、工具栏撤「清空回收站」按钮；
   `AssetLibraryScope` / 侧栏计数 / 主进程 scope 白名单与 SQL 分支里的 trash 一并摘掉。
2. 四个删除入口（卡片右键菜单 / Delete 键 / 大图查看器 ×3 / 重复素材弹窗 ×2）统一走新增的
   **`deleteAssets`**：**无引用冲突当场真删；被其他任务 / 工作区 / Agent 会话引用的挂起弹确认**
   （`AssetPurgeModal`，默认**不勾**「解除引用并彻底删除」）。
3. 删任务卡改回 `purgeTaskOutputAssets`（产出图随卡真删），被其他任务/会话引用的图仍保留并如实报数；
   任务卡输出槽位收尾复用 `patchTaskForPurgedSlots`（卡片仍显示「已删除」）。
4. 存量迁移两处：主进程 `CATALOG_MIGRATIONS` **v2**（`CATALOG_SCHEMA_VERSION` 1 → 2）
   把残留的 `status='trashed'` 素材**恢复成正常素材**；渲染层新增 `normalizePersistedScope`
   把持久化的 `scope='trash'` 归一为 `all`。
5. 数据库表结构**未动**（`assets.status` 列保留，此后只产 `active`）；底层 `moveToTrash` /
   `restoreAssets` 保留实现但**已无 UI 入口**（为将来「撤销删除」留基础）。
6. 文案：HelpModal 两处 + 六处「一并移入回收站（可恢复）」→「一并删除（不可恢复）」；
   冒烟清单 3 处、架构约束 1 处、ADR-0019 加作废抬头。

**验收标准（可测）**

1. 侧边栏只有四项（全部素材 / 最近生成 / 收藏 / 未整理），没有回收站；
2. 任意入口删除一张**未被引用**的图 → 当场消失（记录 + 原图字节 + 缩略图一起清）；
3. 删除一张**被别的任务卡引用**的图 → 弹「解除引用并彻底删除」确认，取消则不删；
4. 删任务卡 → 产出图一并永久删除（不再进回收站）；被引用的那一张仍保留并如实报数；
5. 库里残留的 `status='trashed'` 素材在启动迁移后变回正常素材（不再有看不见的幽灵）；
6. 旧持久化里 `scope='trash'` → 启动后落在「全部素材」；
7. `npm run verify` 全绿。
...
- `npm run verify` **全绿**：tsc 双端零错误 / eslint 零告警 / prettier 全 unchanged /
  **270 文件 3301 用例通过**。
- **反向验证一轮，精确命中 2 条**（两处都已回读校验复原，复原后 262 用例全绿）：
  1. 把 `purgeTaskOutputAssets` 的最后一步改回软删（`moveToTrash`）→ 恰好 1 条红：
     `src/store.test.ts > 把任务卡删掉时，它的产出图一并永久删除`，
     `AssertionError: expected [] to include 'img-cascade'`，其余 261 条照过；
  2. 停掉 `deleteAssets` 里的「挂起待确认」分支 → 恰好 1 条红：
     `deleteAssets 被引用的素材挂起确认，绝不静默 force 强删`，
     `AssertionError: expected null to deeply equal { ids: [ 'b' ], …(2) }`。
- **天然反向证据**：改完后首轮 verify 恰好 1 条红，正是 TB-119 那条
  「把任务卡删掉时，它的产出图移入回收站而不是永久删除」——说明旧行为确实被改掉了，
  已按新语义改写（并补一条「同一原图的导出副本一并清掉」的旧口径断言）。
- 用例调整：`store.test.ts`（删任务卡用例改真删语义）、`features/assetLibrary/store.test.ts`
  **+2**（`deleteAssets` 无冲突当场删 / 有引用挂起且不 force）、`query.test.ts`（撤 trash scope 与计数断言，
  新增「trashed 在任何 scope 都不可见」）、`assetLibraryModel.test.ts` **+1**（5 条断言覆盖
  「已回收素材对任何范围都不可见」）、`AssetBatchView.test.tsx`（把 TB-119 的「回收站放行孤儿组」
  **反向改写**成「孤儿组在任何作用域都不显示」）、`AssetCardMenu.test.tsx` / `AssetDuplicateModal.test.tsx`
  （依赖从 `moveToTrash` 换到 `deleteAssets`、按钮文案改「删除」）、`sopInitialGroup.test.ts`（scopes 去 trash）。
- 新风险：**R-105**（误删不可恢复、无 Ctrl+Z）。

**未做 / 待确认**

1. **本机无法做渲染验证**（环境限制）⇒ 界面改动只做了组件级用例 + 规则推导，**未经真机观感确认**。
   请过目三处：侧边栏是否只剩四项、删一张图是否当场消失、删任务卡后卡片是否显示「已删除」。
2. **连带行为（刻意保留旧口径，若不要请拍板）**：删任务卡时，**别的幸存任务**引用的同一张图的
   `localSavedOutputImagePaths`（用户磁盘上的导出副本）会被一并删掉 —— 这是 ADR-0019 之前的行为，
   理由是原图字节都没了、那些副本只剩死路径。你在意「别动我磁盘上的文件」就说一声，改动很小。
3. **`ImportScopeFields` 的文案仍写着「本机自建的会被移进回收站，可再恢复」** —— 那里说的是
   **文件夹**（collections）的软删位，与已撤除的**素材**回收站是两回事。但目前 UI 里没有恢复文件夹的
   入口，这句话可能是空头承诺；属独立问题，本轮一个字节没碰。
4. 底层软删链路（`moveToTrash` / `restoreAssets` / `applyTrashStatus` / `detachTrashedAssetsFromTasks`）
   **保留未删**（理由见 ADR-0021「为什么不那么做」）。若确认永久不需要，可另开一轮清理。

---

## TB-128 后处理进度弹窗按范围分页（当前方向 / 全部方向）（2026-09-24 阿伟）

**背景**：杰哥 2026-09-24 原话「后处理进度弹窗采用多标签页（tab）方式展示导出记录：默认打开并激活
展示当前方向（当前导出方向）的导出记录标签页，用户可通过切换标签查看全局记录」，并要求先明确四点：
① 标签页命名与默认选中逻辑 ② 各页内容范围 ③ 切换交互 ④ 无记录 / 加载中的状态展示。
（方案先报后做，四点逐条定死，见下；拍板点只有「正在跑的那块是否常驻」一个。）

**为什么改**：改之前面板是**一坨竖排三段**（正在跑 → 批次级结果 → 全部方向的历史）没有任何筛选。
方向一多，第三段就是一长串折叠卡片；而面板是从**素材库工具栏**打开的 —— 用户此刻心里想的是
「我刚在那个方向发的那批到哪了」，界面上却没有任何优先级。

**拍板结论（杰哥 2026-09-24）**：**在飞的进度也跟着标签页过滤，不常驻**。他的原话
「正在跑放在对应方向的 tab 和全部方向，也就是说要不就是从对应方向打开才能看到、要不就是切换到
全体的 tab」。代价（当前方向页看不见别的方向在跑）是**知情选择**，已写进
`architecture-constraints.md` §4.4.3 的「勿改回」清单。

**做了什么**

1. **面板拆两层**：`PostprocessRunsDialog`（只管 `Dialog` 外壳）+ `PostprocessRunsPanel`（主体）。
   拆分的目的是**让标签页状态随打开重置** —— `Dialog` 在 `open=false` 时返回 null、不渲染子树，
   面板每次打开都是新实例，`useState` 初值即「此刻在哪个方向」。不拆就得用 `useEffect` 对齐打开
   事件，还要处理「打开期间方向被删」的竞态。
2. **「当前方向」的判据**与产出目标弹窗**逐字相同**：`scope.kind === 'collection' ? scope.id : null`
   （`PostprocessTargetsDialog.tsx:86` 同一行）。停在「全部 / 收藏 / 未整理 / 标签」时没有当前方向。
3. **标签命名**：当前方向页放**路径末段名**（`月亮`），全路径（`产品线 / 产品 / 月亮`）挂在 `title` 上
   —— `.ds-tabs` 横向可滚动、`.ds-tabs__item` 是 `flex: none` 不截断，三段路径塞进去会把标签条撑成
   滚动条。全部方向页带**条数徽章**（当前方向页里看不见别的方向有没有记录，这是切过去的唯一线索）。
4. **内容范围**：当前方向页 = 这个方向在飞的 + 它的历史；全部方向页 = 全部方向的历史 + 批次级记录
   （「不属于某个方向的结果」只在全部页，它没有方向可归属）+ 所有在飞的。
5. **不在具体文件夹里打开时不渲染标签条**（只有一页可看，摆个单项标签没意义），行为与本轮之前一致。
6. **切换交互**用设计系统现成 `Tabs`（`role=tablist` / 方向键 / roving tabindex），不请求、不改数据。
   切走再切回，方向分组回到「全展开」（既有口径）。
7. **空态收敛 + 修一处旧谎**：原来是**两个空态叠着**（「还没有跑过后处理」+「还没有历史记录」同屏
   说同一件事），现在空只出现一次；并且空态判据**补上落盘历史**——只判内存记录会在**重启后**说谎
   （内存那份清了、盘上历史还在 ⇒ 出现「还没有跑过后处理」与一列历史记录同屏的自相矛盾）。
8. `catalog.ts` 的组件条目更新（职责改「按范围分两页」、targets 加 `Tabs`）。

**验收标准（可测）**

| # | 标准 | 落地 |
| --- | --- | --- |
| 1 | 在素材库某个方向里打开面板，默认选中该方向页，且**只**列它的记录（别的方向的记录不出现） | `⭐ 从某个方向打开时默认停在它的标签页，只看它的记录` |
| 2 | 切到「全部方向」后能看到其余方向的记录 | 同上（用例内点击 tab 后断言） |
| 3 | 正在跑的**跟着页过滤**：当前方向页的进度块只属于这个方向 | `⭐ 正在跑的跟着标签页过滤` |
| 4 | 批次级记录（无方向）只在全部页出现 | `没有方向的批次级结果只在「全部方向」页出现` |
| 5 | 不在具体文件夹里打开时不渲染标签条 | `素材库没停在具体文件夹时不渲染标签条` |
| 6 | 当前方向无记录时空态说的是这个方向，且全局页仍有别的方向的记录 | `当前方向没有任何记录时，空态说的是这个方向` |
| 7 | 一次都没跑过时**只有一个**空态；重启后不说「还没有跑过后处理」 | `一次都没跑过时只有一个空态` / `⭐ 重启后…不说「还没有跑过后处理」` |

**验收证据**

- **`npm run verify` 全绿**：270 文件 / **3308** 用例（基线 3301 → **+7**，正是本轮新增的 7 条）。
- **反向验证两次，都精确命中**：
  ① 把默认页改回恒 `'all'`（撤掉「按 scope 定初值」）→ **4 条红**，正是依赖分页默认值的四条
  （默认停在方向页 / 在飞过滤 / 批次级只在全部页 / 当前方向空态）；
  ② 把在飞的过滤拿掉（`scopedRuns` → `runs`）→ **1 条红**，正是「正在跑的跟着标签页过滤」。
  两处复原后均已 `grep` 回读校验。
- ⚠️ **本机无法做渲染验证**（环境限制，见 runbook 二十七节）：观感**未经真机确认**。

**未做 / 待确认**

1. **请你在 dev 里过目三处**：① 在某个方向里打开面板，是否停在那个方向的页；② 切「全部方向」是否
   看到其余记录；③ 标签名是方向末段名 —— 长名字会不会把标签条撑出横向滚动（真撑了我改成固定文案
   「当前方向」四个字，一行改动）。
2. **停在哪页每次打开重算，不记上次**（理由见文件头注释）。若你希望「记住上次看的那页」，说一声。
3. 在飞跟着页过滤的代价已按你的选择固化，别人（包括我）**不该**为了「不丢信息」把它改回常驻 —— 已登记
   `architecture-constraints.md` §4.4.3。

---

## TB-129 任务卡以任务记录为准：不因图不可见而消失，数量只随手动删除变（2026-09-24 阿伟）

**需求（杰哥原话，2026-09-24）**

1. 任务卡禁止自动消失，数量必须持续存在，只有当我手动删除整张卡片或删除卡内的某些图片时，
   才根据我的操作相应地改变和显示数量；
2. 图片模式与任务卡片模式下的图片存在关联，该关联在生图时即已确定，并以任务卡片为准。

**病根：卡片是「按图建的」，不是「按任务建的」**

`buildAssetBatchGroups` 的主循环体是**素材**（`assetBatchGrouping.ts`），任务只是挂在素材上的
归属标签。于是「任务记录在、它的图暂时查不到」时：

| 场景 | 旧行为 |
| --- | --- |
| 把卡内的图**拖到别的文件夹** | 卡内图变少；图全部拖走 → **卡片凭空消失** |
| 图落在**还没加载的分页**里 | 卡上数量偏小 |
| 搜索词把这张图过滤掉 | 同上（这条是用户主动行为，保留） |

兜底谓词 `includeTaskless` 当时还把**已成功完成**的任务整类 `continue` 掉
（`task.status === 'done' && !hasTaskFailure(task)`）⇒ 成功任务**没有任何兜底**。

数量则是**两套口径**并存（这就是需求 2 说的「关联对不上」）：

| 位置 | 旧口径 |
| --- | --- |
| 卡内角标 | `TaskCard` 的 `liveOutputCount` = 任务记录里还剩几张 ✅ 已经是对的 |
| 卡外 / 分组头 / 速览 | `group.assets.length` = **当前能看见几张素材** ❌ 会变 |

**改了什么**

| # | 文件 | 改动 |
| - | ---- | ---- |
| ① | `src/lib/assetBatchGrouping.ts` | 建卡口径：`done` 任务改为**「有产出就建卡」**（只有「已完结且从未产出」的历史空任务才跳过，保住「不被历史任务刷屏」）。新增 `countLiveTaskOutputs()` 作「还剩几张」的**唯一实现**；`AssetBatchGroup` 新增 `outputCount` 字段，在 **SOP 去重之后**统一算一次 |
| ② | 同上 | 速览 `assetCount` 改用 `group.outputCount`（与卡片同口径），不再退回 `assets.length` |
| ③ | `src/features/assetLibrary/AssetBatchView.tsx` | 两处 `aria-label`、组头「N 张」、批次进度 `imageCompleted` 全部改读 `outputCount` |
| ④ | `src/components/TaskCard.tsx` | `liveOutputCount` 收敛为调用 `countLiveTaskOutputs`；该文件内另外 3 处 `.length` 也统一（同一张卡上两个数字的来源只剩一个） |

**为什么 `countLiveTaskOutputs` 必须 `filter(Boolean)`**：`outputImages` 是**定长槽位数组** ——
用户删掉卡内某张图时，`patchTaskForPurgedSlots`（`lib/assetPurge.ts`）把该槽位置成 `undefined`
但**数组长度不变**，并记进 `purgedOutputSlots`。用 `.length` 会把已删的槽位算进去，
报出一个点不开的数字。这正是需求 1「删卡内图片才变」的落点：删图 → 槽位空 → 数量 −1。

**口径边界（刻意留的兜底）**

- **图被拖到别的文件夹 → 卡片留在原文件夹不动**（杰哥 2026-09-24 拍板）：
  卡片归属看任务的 `defaultCollectionId`（提交时定），不看图现在落在哪。切到 B 文件夹看不到这张卡，
  但图片模式里能正常看到那张图。
- **只降不升的兜底**：任务记录算出来是 0、而卡上确实挂着图 ⇒ 退回可见素材数。正常数据下任务记录必然有产出
  （素材就是从它的 `outputImages` 同步过去的），所以这条**正常路径永不触发**、不破坏稳定性；
  它只为「任务记录缺产出信息」（迁移不全 / 落盘不完整）时兜住，避免出现「卡上写 0 张、点进去明明有图」。
- **整卡删除不受影响**：`handleDeleteGroup` 走的是 `removeTask` / `removeMultipleTasks`（按**任务**删），
  本来就与 `group.assets` 无关，所以没有「漏删」问题。

**验收证据**

- **`npm run verify` 全绿**：270 文件 / **3313** 用例（基线 3308 → **+5**，正是本轮新增的 5 条）。
- **反向验证（撤掉修复 → 精确红）**：把建卡条件改回 `if (done && !failure) continue`、
  并把 `outputCount` 改回 `assets.length` → **恰好 7 条红**，其中 4 条正是本轮新增的守卫用例
  （`有产出的已完成任务：图不在查询结果里也建卡` / `卡内的图一张都查不到时，卡片仍然在` /
  `速览的图片数与卡片同一个口径` / `没有素材但有任务记录时，显示任务卡而不是空态`），
  另 3 条是空态用例失败后的**连带污染**（它断言失败 → `unmount()` 没跑到 → 后面的定位/高亮用例被带红，
  同一根因）。56 条绿。复原后已 `grep` 回读校验。
- **顺手补掉一个测试环境缺口**：`AssetBatchView.test.tsx` 的 store mock 缺
  `resolveImageDisplaySrc`（`TaskCard` 的缩略图兜底会调它）。它表现为 **unhandled rejection**：
  一旦有用例真的渲染出卡片就抛「No "…" export is defined on the "../../store" mock」，
  并以**莫名其妙的连带失败**形式把后面的用例带红（本轮先被它骗过一次）。
- ⚠️ **本机无法做渲染验证**（环境限制，见 runbook 二十七节）：观感**未经真机确认**。

**未做 / 待确认**

1. **请你在应用里过目四处**：① 把一张图从 A 文件夹拖到 B → 卡仍在 A、数量不变；
   ② 从卡片删掉一张图 → 数量 −1；③ 删整张卡 → 卡消失；④ 任务已完成但图不在当前查询结果里 → 卡仍在。
2. **右键整卡的「全选组内」仍以当前可见素材为目标**（`handleGroupMenu` 用 `group.assets`）：
   卡上写 5 张而只查得到 2 张时，右键批量只会作用于那 2 张。这是**既有的**语义（「作用于这批图」），
...
## TB-130 导出位置开关：位置配置保留、临时不写（2026-09-24 阿伟）
...
**病根：想「暂时别往那边写」只有删位置这一条路，而删了配置就没了**

共享盘路径**手输不在主进程白名单里**（`assertAllowedPath` 只认「选择目录」选过的目录）——
盘不可达时连选都选不回来，删掉就等于要等盘恢复后重新配一遍。

**做了什么**（决策与理由见 [ADR-0022](adr/0022-output-dir-write-toggle.md)）

1. **位置列表 `mediaOutputDirs` 一个字节不改**，开关另存一份「路径 → 开/关」的表
   （全局 `mediaOutputDirEnabled[mediaId]`、节点 `byMedia[m].outputDirEnabled`）。
   做成 `{path, enabled}[]` 会要求逐槽位继承 —— 那会让「本级填一处」变成「本级那处 + 继承的所有处」，
   一次升级就多写文件。
2. **三态**：缺键 = 沿用浅一层（链上都没有 = 开）；`false` = 停用；`true` = 盖掉浅一层的 `false`
   （「全局关了、这个方向想用」）。少了 `true` 这档，本级点开被上级停用的开关会**没反应**。
   逐层**逐键浅合并**（不是整份替换）—— 整份替换时「全局停了共享盘、这个方向再停本地」
   会把全局那条抹掉，共享盘自己又开始写了。
3. 两张表（中控台 `ConsoleMediaTables`、后处理弹窗 `ChannelOutputDirs`）各加一列
   **「写入」**。**刻意不叫「启用」**：表里已经有一个渠道级开关「参与产出」，
   撞名会让人以为是同一件事的两种说法（ADR-0013 删掉渠道级 `enabled` 就是这个原因）。
   两者一句话能分开：关「参与产出」= 这个渠道**不产出变体**；关「写入」= 变体照产出、只是**不落这一处**。
4. **写盘**：停用的位置不进清单 —— 不建目录、不写文件、不参与分发、也不再报「目录不可用」。
   ⚠️ **「配了位置但全停」与「一处都没配」必须分开判**（两条分支都是一样的空数组、意思正相反）：
   前者一处都不写并报 `PP-DIR-005`（severity `skipped`，不是故障），后者仍用默认输出位置。
   合在一起会让「把交付目录全关掉」变成**悄悄写到本地**。报 `PP-DIR-003` 时分母也用过滤后的数量。
5. **继承来的位置也能逐处开关**（杰哥 2026-09-24 选定的口径）：本级留空时按**能继承到的位置数铺行**，
   于是「只停继承来的第二处、第一处照写」这种状态显示得出来、也做得到。铺行规则抽成
   `planPostprocessOutputDirSlots`，两个入口共用一份（各写一遍迟早分叉成「一个表两行、另一个一行」）。
   **不对称（刻意的）**：全局作用域留空那行**不给**开关 —— 全局层停兜底位置等于「这个渠道不产出」，
   那件事归「参与产出」管，不造第二个入口。
6. **路径改名时开关记录跟着搬**；删位置 / 删渠道时清掉记录（不清的话，以后填回同一路径会莫名是关的）。
7. **Excel 往返不加开关列**（`output_dirs_global` / `output_dirs_node`），导入**保留**既有记录；
   **配置包**（`treeConfigBundle`）则带上开关 —— 它是整份配置搬家，漏一处就是静默少一份状态。
8. 落盘版本**刻意不动**（`tangbao-postprocess-media` 保持 v4）：纯新增字段、缺键即旧行为，
   没有旧语义要折算；但 `partialize` 白名单必须加（漏了 = 点完开关当场生效、重启全变回开）。

**验收标准（可测）**

| # | 标准 | 落地 |
| --- | --- | --- |
| 1 | 关掉一处 → 另一处照写，且**不报**「少写了几个位置」 | `停用一处 → 只写剩下那一处，且**不报**…` |
| 2 | 全部关掉 → 一处都不写、报「已跳过」、**不回退默认位置** | `⭐ 全部停用 → 空列表 + PP-DIR-005，**绝不**回退去写默认位置` |
| 3 | 改路径后那处仍然关着（记录跟着搬） | `改路径时开关记录跟着搬（不搬的话…）` |
| 4 | 删位置只清掉它的记录，别处的停用留着 | `⭐ 整份重写只保留「还在的位置」的记录…` |
| 5 | 上一级关上、本级能重新打开 | `⭐ 本级写 true 能盖掉浅一层的 false…` |
| 6 | 本级只停继承来的一处，**全局一个字节不动** | `⭐ 本级留空、上级配了位置 → 那行也能关，且只写进本级…` |
| 7 | 上级两处时本级铺两行，能只停其中一处 | `⭐ 上级有两处时本级铺两行，能只停其中一处…` |
| 8 | 没有路径的行开关置灰 | `没有位置的行开关是灰的（开关没有作用对象）` |

**验收证据**

- **`npm run verify`**：`tsc` 双端 + `lint` + `format:check` **全绿**；`test` **270 文件 / 3342 用例**
  （基线 3313 → **+29**，正是本轮新增的 29 条：核心库 10 / store 9 / 写盘 5 / 中控台 UI 5）。
- ⚠️ **同一次 verify 里有 1 条失败，与本次改动无关**：`electron/legacy-data-migration.test.ts`
  的 `beforeEach`（`rmSync`/`mkdirSync`）在 270 个文件并跑时 **hook 超 10s**。
  单独跑该文件 **19 passed** —— 属满负载下的环境 flaky，已复现确认。
- **反向验证（撤掉开关过滤 → 精确红）**：把 `resolveBucketOutputRoots` 的
  `configured.filter((dir) => isOutputDirEnabled(config, mediaId, dir))` 改回 `configured`
  → **恰好 4 条红**，正是依赖过滤的四条（停一处 / 全停 / 填过的默认目录可停 / 停用的不参与不可用判定）；
  其余 7 条绿。复原后已 `grep` 回读校验。
- ⚠️ **本机无法做渲染验证**（环境限制，见 runbook 二十七节）：观感**未经真机确认**。

**未做 / 待确认**

1. **请你在 dev 里过目三处**：① 中控台「渠道与输出」表里新那一列「写入」；② 关掉某渠道的共享盘
   位置后跑一次后处理 —— 产物只落另一处；③ 把某渠道两个位置都关掉 → 结果里是「已跳过」而不是失败。
2. **列名叫「写入」而不是「启用」**是我为避免与「参与产出」撞名定的。你要是觉得「停用 / 启用」
   更顺手，改文案即可（`ConsoleMediaTables.tsx` / `ChannelOutputDirs.tsx` 各一处）。
3. **本地保存目录下的 `postprocess`**（谁都没填过时的程序兜底）**停不了** —— 它的路径是运行期
   解析出来的，开关表里没有它可指的字符串。要「这个渠道别产出」请取消勾选「参与产出」。
4. 这条口径与 TB-128（进度弹窗）/ TB-129（任务卡）同期落地，三者的未提交改动混在同一工作区 ——
   提交时**只 add 本轮文件**（R-09 / R-79）。
