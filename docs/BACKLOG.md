...
**⭐ 先纠偏：这是两处不同的地方，别当成同一件事**

| 时间  | 截图           | 症状                       | 正确改法                     | 条目   |
| ----- | -------------- | -------------------------- | ---------------------------- | ------ |
| 21:32 | **配方卡面板** | 面板只占内容高，下方空一截 | 内层控件**撑满**固定高容器   | TB-055 |
| 21:36 | **后处理弹窗** | 弹窗写死 80dvh，内容少就空 | 外层容器高度**随内容自适应** | TB-057 |

两者方向相反但不矛盾：容器高度**固定**时（编辑器卡片）要让内容撑满；
容器高度**本来自由**时（弹窗）就该让容器贴合内容。
