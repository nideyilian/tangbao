...
| **P4** | **审核规则**作为产品级参数             | 只有配方卡 `adNegativeRules`                                                             | **需先确认业务需求** |
...
1. **是否做三栏同屏**（P1）？改动最大、收益最大，直接解 TB-049 的入口问题
2. **配置模板要不要**？若要，**模板与节点直接覆盖谁优先**？
   （⚠️ 注意：模板是**增加一层间接**，与 ADR-0011 的收窄方向有张力。
   建议只做「从现有节点反向存成模板」+「套用」，不做「所有节点都必须走模板」）
3. **「审核规则」在糖包有真实业务需求吗**？（有才做，不照搬）
4. **灵境与糖包是否要打通**（同一份数据双向同步）？还是各自独立维护？
