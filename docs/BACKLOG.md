...
- **剩余（按依赖顺序）**
  1. P2 其余 A 档区域：方向表、方向级参数表、全局输出位置表、方向 × 渠道覆盖表、
     方向 × 预设归属表、分发配置、命名与署名、产出清单
  2. P3 导出：xlsx 12 张 Sheet（落点必须走保存对话框 —— 主进程路径白名单会拒手输的 `D:\…`）
  3. P3 导入：三模式 + dry-run + 回滚 + §4.3 六条一致性校验
  4. P4 水印预设主子表（B 档；画布编辑保留，表格只管批量改名 / 开关 / 挪位）
