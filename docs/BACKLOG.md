...
### TB-050 参数模型重构：项目树降级为继承载体，参数改「全局模块 + 项目×方向索引表」

- **来源**：杰哥原话（2026-09-20）「当前参数设置是基于项目树的层级结构。我希望重构为全局的参数设置模块：
  ① 将项目树拓展为全局参数设置模块，统一管理各项目、各方向的参数；② 后处理环节只保留与「对应项目」和
  「对应方向」直接相关的必要设置项，移除冗余层级和无关参数；③ 请评估：后处理在只保留必要设置后，
  是否仍需要树形结构？如不需要，请推荐更合适的组织方式（如扁平化配置、按项目+方向索引的字典/表结构），
  并说明重构后的参数模型、数据流及主要代码调整点。」
- **状态**：📋 **已评估，待杰哥裁决 3 点后开工** · 写线：主写线
- **决策记录**：`docs/adr/0009-params-global-module-vs-tree.md`（提案中）

#### 评估结论（与要求的冲突点先说）

| 你的要求                    | 评估结论                                                                                                                                                                              |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ① 项目树 → 全局参数设置模块 | **方向对，但要拆成两件事**：`ProjectTreeWorkbench` 是**结构**、`PostprocessSettingsModal` 是**参数**。建议新建 `ParamCenterModal` 分两 tab 承载，底层仍两棵树两 store                 |
| ② 后处理只留必要项          | **可行，但「必要」要按证据定**：11 项里 5 项上收全局（`autoCompanionClean` / `distribution` / `selectedMediaIds` 等**无逐方向差异证据**）→ `PostprocessNodeOverride` 10 字段 → 5 字段 |
| ③ 还需要树吗？              | **参数编辑不需要树，但树不能删**：继承链（方向→产品→产品线→全局）是真实存在的，删了会丢继承。树降级为「批量编辑表格里的一个列」                                                       |

#### 关键事实（决定了不能像纯扁平化那样做）

- **`undefined` = 继承 / 空值 = 显式覆盖**（ADR-0004）是用户可推理的语义，纯扁平字典会**丢掉继承** →
  改产品线目录要遍历所有方向循环写，新增方向无默认值。
- **「冗余」的准确位置不是树，是可覆盖字段集开得比业务需求宽**：ADR-0003 记录 61 个方向里
  只有 **25 个目录分叉、56 个水印分叉**，其余 8 个字段**无任何业务证据**表明逐方向不同。
- 存储形状 `Record<collectionId, {postprocess}>` **完全不变** → **零迁移成本**，`version` 不用 bump。
- ⚠️ 唯一有数据风险的一步：旧数据里节点级的 `distribution` / `autoCompanionClean`，
  处置方案推荐 **C（提升到全局 + 提示用户）**，需一次性写盘迁移 + 备份（R 级风险）。

#### 命名撞车（需裁决）

「方向」现在指两件事：① 结构树第 3 层的**节点名**（`{direction}` token 取它）；
② 配置项 `direction`（`landscape`/`portrait`/`square`，决定产出哪些尺寸）。
建议 ② 改名「**产出尺寸方向**」并上收全局。

#### 前置依赖

**必须先修 TB-049**。本次重构会大面积动 `PostprocessNodeOverride` 与消费端；
白名单 bug 还在的话，重构完看到的仍是「导出位置不可用」，**分不清是新引入还是老 bug**。

#### 待裁决 3 点

1. 是否接受「结构树保留、只把参数做成索引表」？（不接受则需接受两个代价，见 ADR）
2. 「方向」改名「产出尺寸方向」并上收全局，是否同意？
3. 旧数据节点级 `distribution` / `autoCompanionClean` 走方案 C（提升+提示）还是 B'（保留不显示）？
