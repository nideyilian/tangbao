...
**P1 / P2 / P3 / P4 全部落地（2026-09-23 ~ 09-24）。**
...
| P1  | 已完成 | **TB-125**：`TaskRecord` 加 `interruptedAt`（只由启动时写、`executeTask` 开始即清）；卡片从「与用户主动停止共用一张脸（已停止）」改为单说「上次没跑完」；按钮从「重试」变「继续」（`continueInterruptedTask` 走**启动恢复同一条路**，不重复产图）。`verify` EXIT=0 / 270 文件 / 3279 用例；反向验证精确命中 2 条。<br>**刻意未动**：批量编排批次的「启动即自动续跑」保持原样（改成"先问"属能力回退，待另行拍板）。                                                                                                                                |
